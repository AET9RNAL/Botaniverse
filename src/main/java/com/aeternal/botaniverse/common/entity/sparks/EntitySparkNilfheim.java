...
public class EntitySparkNilfheim extends Entity implements ISparkEntity {

    private static final int TRANSFER_RATE_NILFHEIM = Config.SparkNilfheimConductivity;
    private static final String TAG_UPGRADE = "upgrade";
    private static final String TAG_INVIS = "invis";
    private static final DataParameter<Integer> UPGRADE = EntityDataManager.createKey(EntitySparkNilfheim.class, DataSerializers.VARINT);

    private final Set<ISparkEntity> transfers = Collections.newSetFromMap(new WeakHashMap<>());

    private int removeTransferants = 2;

    public EntitySparkNilfheim(World world) {
        super(world);
        isImmuneToFire = true;
    }
    ...
    public ItemStack getPickedResult(RayTraceResult target) {
        return new ItemStack(ModVItems.spark_nilfheim);
    }
    ...
    public void onUpdate() {
        ...
        if(upgrade == SparkUpgradeType.DOMINANT || upgrade == SparkUpgradeType.RECESSIVE)
            allSparks = AdditionalSparkHelper.getSparksAroundNilfheim(world, posX, posY + (height / 2.0), posZ);

        Collection<ISparkEntity> transfers = getTransfers();

        switch(upgrade) {
            case DISPERSIVE : {
                List<EntityPlayer> players = AdditionalSparkHelper.getEntitiesAroundNilfheim(EntityPlayer.class, world, posX, posY + (height / 2.0), posZ);

                Map<EntityPlayer, TObjectIntHashMap<ItemStack>> receivingPlayers = new HashMap<>();

                ItemStack input = new ItemStack(ModVItems.spark_nilfheim);
                for(EntityPlayer player : players) {
                    ...
                    for(ItemStack stack : stacks) {
                        ...
                        if(manaItem.canReceiveManaFromItem(stack, input)) {
                            ...
                            int recv = Math.min(getAttachedTile().getCurrentMana(), Math.min(TRANSFER_RATE_NILFHEIM, manaItem.getMaxMana(stack) - manaItem.getMana(stack)));
                            if(recv > 0) {
            ...
            case DOMINANT : {
                ...
                for(ISparkEntity spark_nilfheim : allSparks) {
                    if(spark_nilfheim == this)
                        continue;

                    SparkUpgradeType upgrade_ = spark_nilfheim.getUpgrade();
                    if(upgrade_ == SparkUpgradeType.NONE && spark_nilfheim.getAttachedTile() instanceof IManaPool)
                        validSparks.add(spark_nilfheim);
                }
        ...
        if(!transfers.isEmpty()) {
            int manaTotal = Math.min(TRANSFER_RATE_NILFHEIM * transfers.size(), tile.getCurrentMana());
            int manaForEach = manaTotal / transfers.size();
            int manaSpent = 0;

            if(manaForEach > transfers.size()) {
                for(ISparkEntity spark_nilfheim : transfers) {
                    if(spark_nilfheim.getAttachedTile() == null || spark_nilfheim.getAttachedTile().isFull() || spark_nilfheim.areIncomingTransfersDone()) {
                        manaTotal -= manaForEach;
                        continue;
                    }

                    ISparkAttachable attached = spark_nilfheim.getAttachedTile();
                    int spend = Math.min(attached.getAvailableSpaceForMana(), manaForEach);
                    attached.recieveMana(spend);
                    manaSpent += spend;

                    particlesTowards((Entity) spark_nilfheim);
                }
    ...
    private void dropAndKill() {
        SparkUpgradeType upgrade = getUpgrade();
        entityDropItem(new ItemStack(ModVItems.spark_nilfheim), 0F);
        ...
        if(!isDead && !stack.isEmpty()) {
            ...
            if(stack.getItem() == ModItems.twigWand) {
                if(player.isSneaking()) {
                    ...
                    for(ISparkEntity spark_nilfheim : AdditionalSparkHelper.getSparksAroundNilfheim(world, posX, posY + (height / 2.0), posZ))
                        particleBeam(player, this, (Entity) spark_nilfheim);
                    return true;
                }
    ...
    private void filterTransfers() {
        ...
        while (iter.hasNext()) {
            ISparkEntity spark_nilfheim = iter.next();
            SparkUpgradeType upgr = getUpgrade();
            SparkUpgradeType supgr = spark_nilfheim.getUpgrade();
            ISparkAttachable atile = spark_nilfheim.getAttachedTile();

            if(!(spark_nilfheim != this
                    && !spark_nilfheim.areIncomingTransfersDone()
