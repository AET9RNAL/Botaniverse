...
import static com.aeternal.botaniverse.common.item.ModVItems.spark_asgard;

public class EntitySparkAsgard extends Entity implements ISparkEntity {

    private static final int TRANSFER_RATE_ASGARD = Config.SparkAsgardConductivity;
    private static final String TAG_UPGRADE = "upgrade";
    private static final String TAG_INVIS = "invis";
    private static final DataParameter<Integer> UPGRADE = EntityDataManager.createKey(EntitySparkAsgard.class, DataSerializers.VARINT);

    private final Set<ISparkEntity> transfers = Collections.newSetFromMap(new WeakHashMap<>());

    private int removeTransferants = 2;

    public EntitySparkAsgard(World world) {
        super(world);
        isImmuneToFire = true;
    }
    ...
    public ItemStack getPickedResult(RayTraceResult target) {
        return new ItemStack(spark_asgard);
    }
    ...
    public void onUpdate() {
        ...
        if(upgrade == SparkUpgradeType.DOMINANT || upgrade == SparkUpgradeType.RECESSIVE)
            allSparks = AdditionalSparkHelper.getSparksAroundAsgard(world, posX, posY + (height / 2.0), posZ);

        Collection<ISparkEntity> transfers = getTransfers();

        switch(upgrade) {
            case DISPERSIVE : {
                List<EntityPlayer> players = AdditionalSparkHelper.getEntitiesAroundAsgard(EntityPlayer.class, world, posX, posY + (height / 2.0), posZ);

                Map<EntityPlayer, TObjectIntHashMap<ItemStack>> receivingPlayers = new HashMap<>();

                ItemStack input = new ItemStack(spark_asgard);
                for(EntityPlayer player : players) {
                    ...
                    for(ItemStack stack : stacks) {
                        ...
                        if(manaItem.canReceiveManaFromItem(stack, input)) {
                            ...
                            int recv = Math.min(getAttachedTile().getCurrentMana(), Math.min(TRANSFER_RATE_ASGARD, manaItem.getMaxMana(stack) - manaItem.getMana(stack)));
                            if(recv > 0) {
            ...
            case DOMINANT : {
                ...
                for(ISparkEntity spark_asgard : allSparks) {
                    if(spark_asgard == this)
                        continue;

                    SparkUpgradeType upgrade_ = spark_asgard.getUpgrade();
                    if(upgrade_ == SparkUpgradeType.NONE && spark_asgard.getAttachedTile() instanceof IManaPool)
                        validSparks.add(spark_asgard);
                }
        ...
        if(!transfers.isEmpty()) {
            int manaTotal = Math.min(TRANSFER_RATE_ASGARD * transfers.size(), tile.getCurrentMana());
            int manaForEach = manaTotal / transfers.size();
            int manaSpent = 0;

            if(manaForEach > transfers.size()) {
                for(ISparkEntity spark_asgard : transfers) {
                    if(spark_asgard.getAttachedTile() == null || spark_asgard.getAttachedTile().isFull() || spark_asgard.areIncomingTransfersDone()) {
                        manaTotal -= manaForEach;
                        continue;
                    }

                    ISparkAttachable attached = spark_asgard.getAttachedTile();
                    int spend = Math.min(attached.getAvailableSpaceForMana(), manaForEach);
                    attached.recieveMana(spend);
                    manaSpent += spend;

                    particlesTowards((Entity) spark_asgard);
                }
    ...
    private void dropAndKill() {
        SparkUpgradeType upgrade = getUpgrade();
        entityDropItem(new ItemStack(ModVItems.spark_asgard), 0F);
        ...
        if(!isDead && !stack.isEmpty()) {
            ...
            if(stack.getItem() == ModItems.twigWand) {
                if(player.isSneaking()) {
                    ...
                    for(ISparkEntity spark_asgard : AdditionalSparkHelper.getSparksAroundAsgard(world, posX, posY + (height / 2.0), posZ))
                        particleBeam(player, this, (Entity) spark_asgard);
                    return true;
                }
    ...
    private void filterTransfers() {
        ...
        while (iter.hasNext()) {
            ISparkEntity spark_asgard = iter.next();
            SparkUpgradeType upgr = getUpgrade();
            SparkUpgradeType supgr = spark_asgard.getUpgrade();
            ISparkAttachable atile = spark_asgard.getAttachedTile();

            if(!(spark_asgard != this
                    && !spark_asgard.areIncomingTransfersDone()
