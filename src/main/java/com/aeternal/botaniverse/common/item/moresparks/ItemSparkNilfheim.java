...
public class ItemSparkNilfheim extends ItemMod implements IManaGivingItem {

    public ItemSparkNilfheim() {
        super(LibItemNames.SPARK_NILFHEIM);
    }
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing side, float xv, float yv, float zv) {
        ...
        if(tile instanceof ISparkAttachable) {
            ...
            if(attach.canAttachSpark(stack) && attach.getAttachedSpark() == null) {
                if(!world.isRemote) {
                    stack.shrink(1);
                    EntitySparkNilfheim spark = new EntitySparkNilfheim(world);
