...
public class TileMorePool extends TileMod implements IManaPool, IKeyLocked, ISparkAttachable, IThrottledPacket, ITickable {
    ...
    public boolean shouldRefresh(World world, BlockPos pos, @Nonnull IBlockState oldState, @Nonnull IBlockState newState) {
        if(oldState.getBlock() != newState.getBlock())
            return true;
        if(oldState.getBlock() != ModVBlocks.morepool || newState.getBlock() != ModVBlocks.morepool)
            return true;
        return oldState.getValue(BotaniverseStateProps.MOREPOOL_VARIANT) != newState.getValue(BotaniverseStateProps.MOREPOOL_VARIANT);
    }
    ...
    public void update() {
        if (manaCap == -1) {
            MorePoolVariant currentVariant = world.getBlockState(getPos()).getValue(BotaniverseStateProps.MOREPOOL_VARIANT);
            switch (currentVariant) {
                case NILFHEIM:
                    manaCap = MAX_MANA_NILFHEIM;
                    break;
                case MUSPELHEIM:
                    manaCap = MAX_MANA_MUSPELHEIM;
                    break;
                case ALFHEIM:
                    manaCap = MAX_MANA_ALFHEIM;
                    break;
                case ASGARD:
                    manaCap = MAX_MANA_ASGARD;
                    break;
            }
        }
        ...
        for(EntityItem item : items) {
            ...
            if(!stack.isEmpty() && stack.getItem() instanceof IManaItem) {
                ...
                if(outputting && mana.canReceiveManaFromPool(stack, this) || !outputting && mana.canExportManaToPool(stack, this)) {
                    ...
                    int transfRate = Config.TransferIN_OUT_Global * (bellowCount + 1);
    ...
    public void renderHUD(Minecraft mc, ScaledResolution res) {
        ItemStack pool = new ItemStack(ModVBlocks.morepool, 1, world.getBlockState(getPos()).getValue(BotaniverseStateProps.MOREPOOL_VARIANT).ordinal());
        String name = I18n.format(pool.getTranslationKey().replaceAll("tile.", "tile." + Constants.PREFIX_MOD) + ".name");
        int color = 0x4444FF;
    ...
    public int getMaxManaCapacity(MorePoolVariant poolVariant) {
        switch (poolVariant) {
            case NILFHEIM:
    ...
    public int getCurrentMana() {
        if (world != null) {
            ...
            if (state.getProperties().containsKey(BotaniverseStateProps.MOREPOOL_VARIANT)) {
                MorePoolVariant poolVariant = state.getValue(BotaniverseStateProps.MOREPOOL_VARIANT);
                int maxCapacity = getMaxManaCapacity(poolVariant);
                // If you have a mana field in your class, set its value here
                // mana = ...; // Set the mana value here
                return mana;
            }
        }
    ...
    public enum MorePoolType {
        ...
        ALFHEIM,
        ASGARD,
        UNKNOWN // Or a default value to handle unexpected cases
    }
    public MorePoolType getPoolType() {
        IBlockState state = world.getBlockState(getPos());
        if (state.getProperties().containsKey(BotaniverseStateProps.MOREPOOL_VARIANT)) {
            MorePoolVariant variant = state.getValue(BotaniverseStateProps.MOREPOOL_VARIANT);
            switch (variant) {
                case NILFHEIM:
