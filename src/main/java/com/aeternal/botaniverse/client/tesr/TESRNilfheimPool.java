...
//mport vazkii.botania.common.block.tile.mana.TilePool;
...
public class TESRNilfheimPool extends TESR<TileMorePool> {
    ...
    public void renderTileEntityAt(TileMorePool pool, double d0, double d1, double d2, float f, ResourceLocation destroyStage, float alpha) {
        ...
        // Your existing translation and rotation logic
        ...
        TileMorePool.MorePoolType poolType = pool.getPoolType();
        if (pool != null) {
        ...
        switch (poolType) {
            case NILFHEIM:
                bindTexture(TEXTURE_NILFHEIM);
                break;
            case MUSPELHEIM:
                bindTexture(TEXTURE_MUSPELHEIM);
                break;
            case ALFHEIM:
                bindTexture(TEXTURE_ALFHEIM);
                break;
            case ASGARD:
                bindTexture(TEXTURE_ASGARD);
                break;
            default:
                // Handle the default case or any other scenarios
                break;
        }
